...
namespace Algoritmos
{
    public class CFloodFill
    {
        ...
        /// <summary>
        /// Verifica si un punto está dentro de un círculo usando la ecuación: (x-cx)² + (y-cy)² <= r²
        /// </summary>
        private bool EstaDentroDelCirculo(int x, int y)
        {
            // Obtener el centro del canvas (donde está el círculo)
            int centroX = canvasPixelCols / 2;
            int centroY = canvasPixelRows / 2;

            // Obtener el color del borde (rojo) para calcular el radio aproximado
            // Buscamos el punto rojo más lejano del centro para estimar el radio
            int radioEstimado = EstimarRadioCirculo(centroX, centroY);

            if (radioEstimado == 0)
                return true; // Si no se puede estimar, permitir pintar

            // Calcular distancia del punto al centro
            double distancia = Math.Sqrt(Math.Pow(x - centroX, 2) + Math.Pow(y - centroY, 2));

            // El punto está dentro si la distancia es menor o igual al radio
            return distancia <= radioEstimado;
        }

        /// <summary>
        /// Estima el radio del círculo buscando puntos rojos desde el centro
        /// </summary>
        private int EstimarRadioCirculo(int centroX, int centroY)
        {
            int radioMax = 0;

            // Buscar en varias direcciones desde el centro
            for (double angulo = 0; angulo < 2 * Math.PI; angulo += Math.PI / 8)
            {
                for (int r = 1; r < Math.Min(canvasPixelCols, canvasPixelRows) / 2; r++)
                {
                    int x = (int)(centroX + r * Math.Cos(angulo));
                    int y = (int)(centroY + r * Math.Sin(angulo));

                    if (x >= 0 && x < canvasPixelCols && y >= 0 && y < canvasPixelRows)
                    {
                        Color color = ObtenerColorPixel(x, y);
                        
                        // Si encontramos un pixel rojo (borde del círculo)
                        if (color.R > 200 && color.G < 100 && color.B < 100)
                        {
                            radioMax = Math.Max(radioMax, r);
                            break;
                        }
                    }
                }
        ...
        private async Task<int> FloodFillIterativo(int x, int y, Color colorOriginal, Color colorRelleno, PictureBox ptbGrafica)
        {
            ...
            while (pila.Count > 0)
            {
                ...
                // VALIDACIÓN CLAVE: Solo pintar si está dentro del círculo
                if (!EstaDentroDelCirculo(px, py))
                    continue;
